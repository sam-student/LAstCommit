...
class SearchProductView(ListView):
    ...
    def get_queryset(self, *args, **kwargs ):
        ...
        if query is not None:
            ...
            queryset = Product.objects.filter(title__icontains=query).filter(price__icontains=query1).filter(
                Color__icontains=query2)
            # print(queryset)
            if queryset:
            ...
            # obj = Product('Populrity_Score')
                ...
                queryset2 = Product.objects.all().filter(title__icontains=query).filter(price__icontains=query1).filter(Color__icontains=query2)
